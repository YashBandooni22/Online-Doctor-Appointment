...
const MyProfile = () => {
  const [userDate, setUserDate] = useState({
    ...
    email: 'ricardjamesswap@gmail.com',
    phoneNo: "+1-212-456-7890",
    address: {
  ...
  return (
    <div className='max-w-lg flex flex-col gap-2  text-sm' >
      <img className='w-36 rounded' src={userDate.image} alt="Profile Pic" />
      {
        isEdit ? (
...
<hr className='bg-zinc-400 h-[1px] border-none' />
          ...
          <p className='font-medium'>Phone Number</p>
          {
            isEdit ? (
              <input
                ...
                type="text"
                value={userDate.phoneNo}
                onChange={(e) => setUserDate((prev) => ({ ...prev, phoneNo: e.target.value }))}
              />
            ) : (
              <p className='text-blue-400'>{userDate.phoneNo}</p>
            )
          }
      ...
      <div>
        ...
        <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700' >
          ...
          {
            isEdit
              ? <input className='max-w-28 bg-gray-100' type="date" value={userDate.dob} onChange={(e) => setUserDate((prev) => ({ ...prev, dob: e.target.value }))} />
